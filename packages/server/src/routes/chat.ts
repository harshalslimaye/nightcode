...
const submitSchema = z.object({
    content: z.string(),
    mode: z.enum(Mode),
    model: z.string().refine(isSupportedChatModel, "Unsupported model ID"),
});
    ...
    try {
        ...
        for await (const part of result.fullStream) {
            ...
            if (part.type === 'text-delta') {
                fullText += part.text;
                const event: ChatStreamEvent = { text: part.text, type: 'text-delta' };
...
const app = new Hono()
    .post('/:sessionId/resume', async (c) => {
        ...
        const resumableMessage: any = getResumableUserMessage(session.messages);

        if (!resumableMessage) {
            return c.json({ error: 'No resumable user message found' }, 409);
        }

        if (!isSupportedChatModel(resumableMessage.model)) {
            return c.json({ error: 'Unsupported model ID ' + resumableMessage.model }, 409);
        }

        if (activeResumeSessionIds.has(sessionId)) {
            return c.json({ error: 'A resume request is already in progress for this session' }, 409);
        }
